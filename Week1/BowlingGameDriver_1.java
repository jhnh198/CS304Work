 ...
 * 
 */
public class BowlingGameDriver_1 {
    public static void main(String[] args){
        BowlingGame John = new BowlingGame("John");
         ...
         for(int frame = 0; frame < bowlingScores.length - 1; frame++){
            ...
            if(!John.shot(bowlingScores[frame][0])) //if first shot is not a strike, check for gutterballs
            {
                John.setGutterBalls(John.shot(bowlingScores[frame][0]));
                John.setGutterBalls(John.shot(bowlingScores[frame][1]));
            }
            ...
            else if(John.shot(frameScore)){   //spare check
                John.setSpares(true);                
