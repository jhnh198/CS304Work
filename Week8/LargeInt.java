...
public class LargeInt
{
  ...
  // Instantiates a large integer as indicated by intString
  {
    ...
    {
      digitChar = intString.charAt(count); //unicode value
      digitInt = Character.digit(digitChar, 10); //radix, base 10
      digitByte = (byte)digitInt; //mask 32, gt top eight
      addDigit(digitByte);
    }
  }
  ...
  public static LargeInt multiplyLists(SpecializedList larger, SpecializedList smaller){
      //set up basic digit placeholders / loop boundaries
      byte digit1;
      byte digit2;
      byte temp;
      byte carry = 0;
      ...
      //setting up digit iteration
      larger.resetBackward();
      smaller.resetBackward();
      
      //start working through lists
      for(int bottomCount = 0; bottomCount < smallerLength; bottomCount++){
         digit2 = smaller.getPriorElement(); // get bottom number
         
         //setting up a temp product list that will be added to the result
         LargeInt tempProduct = new LargeInt(); //reinstantiate list 
         
         //build zero places
         zeroDigitCount = 0;
         while(zeroDigitCount < bottomCount){
            tempProduct.numbers.addFront((byte)0);
            zeroDigitCount++;
         }
                  
         for(int count = 1; count <= largerLength; count++){
            digit1 = larger.getPriorElement();
                        
            temp = (byte)(digit1 * digit2 + carry);
            carry = (byte)(temp / 10);
            tempProduct.numbers.addFront((byte)(temp % 10));
         }
         if (carry != 0)
            tempProduct.numbers.addFront((byte)carry);
         
         if(LargeInt.greaterList(tempProduct.numbers, result.numbers)){
            result.numbers = addLists(tempProduct.numbers, result.numbers);
         } else result.numbers = addLists(result.numbers, tempProduct.numbers);
      }
